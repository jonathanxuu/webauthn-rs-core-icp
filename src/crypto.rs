...
impl COSEKey {
    ...
    // xyz todo: need to change the validate logic
    pub(crate) fn validate(&self) -> Result<(), WebauthnError> {
        match &self.key {
            COSEKeyType::EC_EC2(ec2k) => {
                let mut pubkey_bytes = vec![0x04];
                pubkey_bytes.extend_from_slice(ec2k.x.as_ref());
                pubkey_bytes.extend_from_slice(ec2k.y.as_ref());

                println!("p256 pubkey bytes: {:?}\n", hex::encode(&pubkey_bytes));

                let point = EncodedPoint::from_bytes(&pubkey_bytes).unwrap();
                let _verifying_key = VerifyingKey::from_encoded_point(&point).unwrap();
    ...
    fn get_p256_key(
        ...
        match &self.key {
            COSEKeyType::EC_EC2(ec2k) => {
                ...
                // println!("compute_sha256 {:?}", compute_sha256(verification_data));
                let verification_result = verifying_key.verify(verification_data, &_sig).is_ok();
                println!("verify sig: {:?}", verification_result);
    ...
    pub(crate) fn verify_signature(
        ...
        let pkey_256 = self.get_p256_key(signature, verification_data);
        return pkey_256;
